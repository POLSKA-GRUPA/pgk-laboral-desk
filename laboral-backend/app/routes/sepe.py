...
def generate_contrato_xml(
    ...
    try:
        xml_bytes, mapping, gen_warnings = contrato_xml_generator.generate(
...
def download_contrato_xml(
    ...
    if not employee.nif or not company.cif or not company.ccc:
        raise HTTPException(
            status_code=422, detail="Employee NIF, Company CIF and CCC are required"
        )
